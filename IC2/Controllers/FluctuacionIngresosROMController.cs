/* Nombre: FluctuacionIngresosROMController.cs
*Creado por: Ana Ilse Aguila Rojas
*Fecha: 06/jun/2019
*Descripcion: Reporte de Fluctuación Cambiaria Ingresos  ROM
...
{
    public class FluctuacionIngresosROMController : Controller
    {
        ...
        public JsonResult LlenaPeriodo()
        {
            ...
            try
            {
                var datos = from periodos in db.FluctuacionIngresoROM
        ...
        public JsonResult LlenarGridFluctuacion(DateTime Periodo, int start, int limit)
        {
            ...
            try
            {
                var lDatosConsulta = from fluctuacion in db.FluctuacionIngresoROM
        ...
        {
            string nombreArchivo = "Fluctuación Ingresos ROM " + meses[periodo.Month].Substring(0, 3) + periodo.Year.ToString().Substring(2, 2) + ".xlsx";
            string templatePath = Server.MapPath("~/Plantillas/FluctuacionCambiariaIngresosROM.xlsx");
            object respuesta = null;
            ...
            using (ExcelPackage excelPackage = new ExcelPackage(datafile))
            {
                try
                {
                    ...
                    var lResultadoConsulta = from fluctuacion in db.FluctuacionIngresoROM
                                             where fluctuacion.fecha_Carga == periodo
                                             select new
                                             {
                                                 fluctuacion.id,
                    ...
                    foreach (var lDatosFluctuacion in lResultadoConsulta)
                    {
                        ...
                        worksheet.Cells[("E" + lFila)].Value = lDatosFluctuacion.deudor;
