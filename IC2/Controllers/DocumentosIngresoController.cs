    ...
    public class DocumentosIngresoController : Controller
    {
                                                                                                                         ...
                                                                                                                         string numDocumentoPF)
        {
            ...
            try {
                var datos = from oDatos in db.documentosIngreso
                            ...
                            && DbFiltro.String(oDatos.deudor, deudor)
        ...
        public JsonResult CargarCSV(HttpPostedFileBase archivoCSV, int lineaNegocio)
        {
            ...
            try {
                ...
                pd = pd.Replace('%', ' ');
                var pdSplit = SepararLineas(pd);
                DateTime periodoDocumento = ConvierteFecha(pdSplit[1], char.Parse("-"), "DMY");
                documentosIngreso docIngreso = db.documentosIngreso.Where(x => x.fechaContable == periodoDocumento && x.activo == 1 && x.lineaNegocio == lineaNegocio).FirstOrDefault();
                if( docIngreso != null) {
                    mensajeExistencia = "Los documentos del periodo " + periodoDocumento.Year + " " + meses[periodoDocumento.Month] +  " se han actualizado";
                    db.sp_BorrarCargas_Delete("documentosIngreso", periodoDocumento, lineaNegocio);
                }
                    ...
                    foreach (string ln in lineas) {
                        ...
                        if (lineaSplit.Count() == 25) {
                            ...
                            try {
                                ...
                                documento.fechaFactura = dtFecha;
