...
Ext.onReady(function () {
    ...
    //**********  Modelo de Busqueda
    Ext.define('model_BuscarMoneda',
        {
            extend: 'Ext.data.Model',
            fields: [
                { name: 'Id', mapping: 'Id' },
                { name: 'Moneda1', mapping: 'Moneda1' },
    ...
    //**********  Busca Moneda
    var store_BuscarMoneda = Ext.create('Ext.data.Store', {
        model: 'model_BuscarMoneda',
        storeId: 'idstore_BuscarMoneda',
        autoLoad: true,
        proxy: {
            type: 'ajax',
            url: '../' + VIRTUAL_DIRECTORY + 'Moneda/llenaGrid?lineaNegocio=' + lineaNegocio,
            reader: {
                type: 'json',
                root: 'results',
        ...
        listeners: {
            load: function () {
                var panels = Ext.ComponentQuery.query('#pnl_moneda');
                if (panels.length > 0) {
    ...
    //**********  Borra Moneda
    var store_BorrarMoneda = Ext.create('Ext.data.Store', {
        model: 'model_BuscarMoneda',
        storeId: 'idstore_BorrarMoneda',
        autoLoad: false,
        proxy: {
            type: 'ajax',
            url: '../' + VIRTUAL_DIRECTORY + 'Moneda/borrarMoneda',
            reader: {
                type: 'json',
                root: 'results',
                totalProperty: 'total'
    ...
    //**********  Modifica Moneda
    var store_ModificarMoneda = Ext.create('Ext.data.Store', {
        model: 'model_BuscarMoneda',
        storeId: 'idstore_ModificarMoneda',
        autoLoad: false,
        proxy: {
            type: 'ajax',
            url: '../' + VIRTUAL_DIRECTORY + 'Moneda/modificarMoneda',
            reader: {
                type: 'json',
                root: 'results',
                totalProperty: 'total'
    ...
    var store_ValidaModifica = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: '../' + VIRTUAL_DIRECTORY + 'Moneda/validaModif',
            reader: {
                type: 'json',
                root: 'results',
                totalProperty: 'total'
    ...
    //**********  Selecciona Moneda
    var store_seleccionarMoneda = Ext.create('Ext.data.Store', {
        model: 'model_BuscarMoneda',
        storeId: 'idstore_seleccionarMoneda',
        pageSize: 20,
        autoLoad: false,
        proxy:
        {
            type: 'ajax',
            url: '../' +VIRTUAL_DIRECTORY + 'Moneda/buscarMoneda',
            reader: {
                type: 'json',
                root: 'results',
                totalProperty: 'total'
    ...
    var paginador = new Ext.PagingToolbar({
        id: 'paginador',
        store: store_BuscarMoneda,
        displayInfo: true,
        displayMsg: 'Monedas {0} - {1} of {2}',
        ...
        items: [
            {
                ...
                listeners:
                {
                    change: function (field, newValue, oldValue, eOpts) {
                        ...
                        store_BuscarMoneda.pageSize = cuenta;
                        store_BuscarMoneda.load();
                    }
                }
            }
    ...
    /**********  FORMAS  **********/
    var pnl_Moneda = Ext.create('Ext.form.Panel', {
        itemId: 'pnl_moneda',
        frame: false,
        border: false,
        margin: '0 0 0 6',
        ...
        items: [
            {
                html: "<div style='font-size:25px';>Moneda</div><br/>",
                border: false,
                margin: '0 0 0 10'
            },
            {
                ...
                items: [
                    {
                        ...
                        handler: function () {
                            var store = Ext.StoreManager.lookup('idstore_BuscarMoneda');
                            store.load();
                        },
                        border: false
                    },
            ...
            },
            {
                ...
                selModel:
                {
                    ...
                    listeners:
                    {
                        selectionchange: function (selected, eOpts) {
                            if (eOpts.length == 1) {
                                id = eOpts[0].data.Id;
                                moneda1 = eOpts[0].data.Moneda1;
                                descripcion = eOpts[0].data.Descripcion;

                                var storeSeleccion = Ext.StoreManager.lookup('idstore_seleccionarMoneda');
                ...
                columns: [
                    ...
                    {
                        xtype: "gridcolumn", sortable: true, dataIndex: 'Moneda1', flex: 1, locked: false, text: "Moneda",
                        renderer: function (v, cellValues, rec) {
                            return rec.get('Moneda1');
                        },
                        editor: {
                            xtype: 'textfield'
                        },
    ...
    Ext.EventManager.onWindowResize(function (w, h) {
        pnl_Moneda.setSize(w - 15, h - 255);
        pnl_Moneda.doComponentLayout();
    });

    Ext.EventManager.onDocumentReady(function (w, h) {
        pnl_Moneda.setSize(Ext.getBody().getViewSize().width - 15, Ext.getBody().getViewSize().height - 255);
        pnl_Moneda.doComponentLayout();
    });
                    ...
                    items: [
                        {
                            ...
                            handler: function () {
                                ...
                                if (form.wasValid) {
                                    form.submit({
                                        url: '../' + VIRTUAL_DIRECTORY + 'Moneda/agregarMoneda',
                                        waitMsg: "Nuevo",
                                        params:
                                        {
                                            id: id,
                                            moneda1: Ext.getCmp('txtMoneda1').value,
                                        ...
                                        success: function (form, action) {
                                            ...
                                            store.getProxy().extraParams.Moneda1 = Ext.getCmp('txtMoneda1').value;
                    ...
                    items: [
                        {
                            ...
                            html: "<button class='btn btn-primary' style='outline:none; font-size: 11px' accesskey='g'>Guardar</button>",
                            handler: function () {
                                var store = Ext.StoreManager.lookup('idstore_ModificarMoneda');
                                store.getProxy().extraParams.Id = id;
                                store.getProxy().extraParams.Moneda1 = Ext.getCmp('txtMoneda1').value;
                                store.getProxy().extraParams.Descripcion = Ext.getCmp('txtDescripcion').value;
                                store.load();
    ...
    var grid = pnl_Moneda.down('gridpanel');
    var proxy = grid.getStore().getProxy();
    extraParams = Help.generarExtraParams(grid);
    proxy.extraParams = extraParams;


    permisosElementos('Moneda', 'grp_Moneda', 'btnGuardar', 'btnEditar', 'btnEliminar', 'log');
